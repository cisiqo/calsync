  ...
  */
  function __construct( $base_url, $user, $pass ) {
    ...
    if ( preg_match( '#^(https?)://([a-z0-9.-]+)(:([0-9]+))?(/.*)$#', $base_url, $matches ) ) {
      ...
      ZLog::Write(LOGLEVEL_INFO, sprintf("server [%s],baseurl [%s] protocol[%s] port[%d]", $this->server,$this->base_url,$this->protocol,$this->port));
    }
    else {
...
//        echo "\nTags array............................................................\n";  print_r( $this->xmltags );
  ...
  */
  function DoRequest( $url = null ) {
    ...
    // $headers[] = "Authorization: Digest username=".$this->user;
    /*$headers[] = 'Authorization: Digest username="'.$this->user.'",
    ...
    $nonce = uniqid();
    $qop = 'auth';
    $uri = $this->server.$this->base_url;
    $nc = 0000001;
    $realm = "BaikalDAV";
    $cnonce = "0a4f113b";
    $A1 = md5($this->user.":".$realm.":".$this->pass);
    $A2 = md5($this->requestMethod.":".$uri);
    ...
    ZLog::Write(LOGLEVEL_INFO, $this->httpRequest);
    $this->xmlResponse = '';
    ...
    ZLog::Write(LOGLEVEL_INFO, $this->httpResponseBody);
    $this->headers = array();  // reset the headers array for our next request
//    ZLog::Write(LOGLEVEL_INFO, $response);
