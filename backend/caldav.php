...
class BackendCalDAV extends BackendDiff {
	...
	public function Logon($username, $domain, $password)
	{
        ...
        // ZLog::Write(LOGLEVEL_INFO,implode(' ',$options));
		if (isset($options["PROPFIND"]))
		{
			ZLog::Write(LOGLEVEL_INFO, sprintf("BackendCalDAV->Logon(): User '%s' is authenticated on CalDAV", $username));
            ZLog::Write(LOGLEVEL_INFO, sprintf("BackendCalDAV->Logon(): Domain [%s] User '%s' is authenticated on CalDAV",CALDAV_SERVER . $this->_caldav_path, $username));
			return true;
		}
		else
		{
			// ZLog::Write(LOGLEVEL_INFO, sprintf("BackendCalDAV->Logon(): User '%s' is not authenticated on CalDAV", $username));
            ZLog::Write(LOGLEVEL_INFO, sprintf("BackendCalDAV->Logon(): Domain [%s] User '%s' Passwd '%s' is not authenticated on CalDAV",CALDAV_SERVER . $this->_caldav_path, $username, $password));
			return false;
		}
	}
